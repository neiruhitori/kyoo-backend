              ...
              <div class="col-lg-6">
                <div class="p-5">
                  ...
                  @include('layouts.alert')
                    ...
                    @endif
                  <form method="POST" action="{{ route('adminBranch.device-account.password.update', $token) }}" class="user">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="token" value="{{ $token }}">
                    <h4 class="text-center mb-3">{{ __('Reset Password Device Account') }}</h4>
                      <small>
                          {{ __('Rules') }}:
                          <ul>
