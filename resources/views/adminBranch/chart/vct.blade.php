    ...
    <div class="row">
        <div class="col" style="max-width: 300px;">
            ...
            <div class="form-group">
                ...
                >
                    @foreach ($vcts as $vct)
                        <option value="{{ $vct->id }}">{{ $vct->name }}</option>
                    @endforeach
                </select>
            </div>
...
<script>
    ...
    $(document).ready(async function () {
        ...
        $("#department_id").change(async function (e) {
            $("#vct_id").html(["<option value disabled selected>Loading...</option>"])
            
            const res = await axios.get(`/api/vct/department/${e.target.value}`)
    ...
    async function fetchData() {
        ...
        return await $.ajax('{{ route('admin-branch.report.vct.chart.all') }}', {
            method: 'GET',
            dataType: 'json',
            data: params
        })
