...
<div class="card mb-4">
    <div class="card-body">
        <div class="d-flex justify-content-between mb-3">
            {{-- <div>
                <a href="{{ route('adminCorporate.report.workstation.chart') }}" class="btn btn-outline-dark">
...
<script>
    ...
    $(function () {
        ...
        $("#pdfButton").click(function (e) {
            ...
            window.open("{{ route('adminCorporate.report.workstation.pdf') }}?" + new URLSearchParams(filter))
        })

        $("#excelButton").click(function (e) {
            const filter = getFilterValue($("#report_type").val())
            window.open("{{ route('adminCorporate.report.workstation.excel') }}?" + new URLSearchParams(filter))
        })
    })
    ...
    async function feedTable() {
        ...
        const res = await axios.get('/admin-corporate/report/workstation/all', {
            params: filter
        })
            .then(res => res.data)
            .catch(err => [])
    ...
    function transformResponse(data) {
        return data.map(v => {
            return `<tr>
                <td>${v.workstation.name}</td>
                <td>${v.serviceNames}</td>
